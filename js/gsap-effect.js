...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (dotRow && beats.length > 0) {
    // Genera automáticamente los puntos indicadores basados en los beats encontrados
    dotRow.innerHTML = '';
    beats.forEach((_, index) => {
      ...
      if (index === 0) dot.style.background = '#3B82F6'; // Activo el primero por defecto
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        // Activa las tarjetas y líneas al entrar en pantalla
        const cards = entry.target.querySelectorAll('.fade-card, .reveal-line');
        cards.forEach((el, index) => {
          setTimeout(() => {
            el.classList.add('active');
          }, index * 100); // Efecto escalonado (stagger)
        });
      }
    });
  }, observerOptions);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (menuBtn && navMenu) {
    menuBtn.addEventListener('click', () => {
      ...
      // Cambia el icono de barras (☰) a una cruz (✕) y viceversa
    ...
    // Cierra el menú automáticamente al hacer clic en cualquier enlace interno
