...
if (canvas) {
  // Optimización de la cantidad de partículas para celular
  ...
  // PixelRatio optimizado para celular para evitar sobrecarga de GPU
  const pr = Math.min(devicePixelRatio, isMobile ? 1.0 : 1.75);
  ...
  const uniforms = {
    ...
    uOpacity: { value: 0.8 },
    uColor: { value: new THREE.Color('#0b51bb') },
    uCore: { value: new THREE.Color('#334ec7') },
    uRing0: { value: new THREE.Color('#0c3879') },
    uRing1: { value: new THREE.Color('#0d43a7') },
    uRing2: { value: new THREE.Color('#283ebe') }
  };
  ...
  let composer = null;
  // Desactivamos efectos de Bloom en móvil para garantizar máxima fluidez (FPS)
  if (!isMobile && !reduced) {
  ...
  // Soporte universal para puntero y táctil en celular sin bloquear scroll
